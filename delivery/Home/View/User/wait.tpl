    ...
    <body>
                                ...
                                <form class="form-horizontal" style="padding-bottom:0">
                                    <div class="control-group">
                                        <div class="controls" style="margin-left:20px">
                                            <select name="" class="sort" style="width:80px;height:30px;margin-left:15px">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <div style="text-align:center;margin-bottom:0;margin-top:10px">
                                <span style="font-weight:bold">标题：</span><input type="text" class="modal-title" style="width:200px;height:15px;margin-top:5px">
                            </div>
                            <div style="text-align:center">
                                <span style="font-weight:bold">作者：</span><input type="text" class="modal-title-author" style="width:80px;height:15px;margin-top:5px">
                                &nbsp;&nbsp;&nbsp;
                                提交于：<span class="modal-title-postdate"></span>
                                &nbsp;&nbsp;&nbsp;
                                状态：<span class="modal-title-status"></span>
                            </div>
                        </div>
                        ...
                        <div class="modal-footer">
                            <span class="edit_rowid" style="display:none"></span>
                            <button type="button" class="btn btn-success btn_save">保存</button>
                            <button type="button" class="btn btn-primary btn_cancel" data-dismiss="modal">取消</button>
                        </div>
                    </div><!-- /.modal-content -->
                </div><!-- /.modal-dialog -->
            </div><!-- /.modal -->
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title" style="text-align:center;margin-bottom:0"></h4>&nbsp;<div style="text-align:center">作者：<span class="modal-title-author"></span>&nbsp;&nbsp;&nbsp;提交于：<span class="modal-title-postdate"></span>&nbsp;&nbsp;&nbsp;状态：<span class="modal-title-status mystatus_wait"></span></div>
                        </div>
        ...
        <script>
            $(function(){
                ...
                //点击编辑按钮
                $(".tablebody").on("click",".btn_edit",function(){
                    var all = {$info["all"]|json_encode};
                    var rowid = parseInt($(this).parent().parent().children('.rowid').text());
                    for(var i=0; i<all.length; i++){
                        if(all[i]["id"]==rowid){
                    ...
                    switch (row["statusid"]){
                        case "1":
                            status = "待审核";
                            $(".modal-title-status").removeClass("mystatus_fail");
                            $(".modal-title-status").removeClass("mystatus_pass");
                            $(".modal-title-status").addClass("mystatus_wait");
                            break;
                        case "2":
                            status = "未通过";
                            $(".modal-title-status").removeClass("mystatus_pass");
                            $(".modal-title-status").removeClass("mystatus_wait");
                            $(".modal-title-status").addClass("mystatus_fail");
                            break;
                        case "3":
                            status = "已通过";
                            $(".modal-title-status").removeClass("mystatus_fail");
                            $(".modal-title-status").removeClass("mystatus_wait");
                            $(".modal-title-status").addClass("mystatus_pass");
                            break;
                    }
                    $("#modal_edit .modal-title").val(title);
                    $("#modal_edit .modal-title-author").val(author);
                    var content_width = $("#modal_edit").width()*0.9;
                    $("#modal_edit .mycontent").css("width",content_width);
                    $("#modal_edit .mycontent").css("height",300);
                    $("#modal_edit .modal-title-postdate").text(postdate);
                    $("#modal_edit .modal-title-status").text(status);
                    $(".edit_rowid").text(rowid);
                    var reg = new RegExp("<br/>", "g");
                    content = content.replace(reg, "\n");
                    var reg1 = new RegExp("&nbsp;","g");
                    content = content.replace(reg1, " ");
                    $("#modal_edit .mycontent").text("");
                    $("#modal_edit .tip").text("");
                    $("#modal_edit .mycontent").val(content);
                ...
                //点击保存按钮
                $("#modal_edit .btn_save").click(function(e){
                    if( $.trim($("#modal_edit .modal-title-author").val())=="" || $.trim($("#modal_edit .modal-title").val())==""){
                        layer.msg('亲，标题和作者不能为空！', function(){});
                    }else if( $("#modal_edit .modal-title").val().length>15 ){
                        layer.msg('标题长度不能超过15个汉字', function(){});
                    }else if( $("#modal_edit .modal-title-author").val().length>10 ){
                        layer.msg('作者姓名太长了！', function(){});
                    }else if($("#modal_edit .mycontent").val().length>600){
                        layer.msg('字数超了，请检查！', function(){});
                    }else{
                        var edit_title = $("#modal_edit .modal-title").val();
                        var edit_title_author = $("#modal_edit .modal-title-author").val();
                        var edit_content = $("#modal_edit .mycontent").val();
                        //var reg = new RegExp("\r\n", "g");
                        edit_content = edit_content.replace(/\n/g,"<br/>");
                        var reg1 = new RegExp(" ","g");
                        edit_content = edit_content.replace(reg1,"&nbsp;");
                        var rowid = parseInt($(".edit_rowid").text());
                        $.post('./updateContri', {edit_id: rowid, edit_title: edit_title, edit_author: edit_title_author, edit_content: edit_content}, function(data, status) {
                            if(status!="success"){
                                alert("数据连接出错！");
                            }else{
                                if(data==1){
                                    $("#modal_edit").modal("hide");
                                    notie.alert(1, '修改成功，即将跳转...');
                                    setTimeout(function(){
                ...
                //点击删除按钮
                $(".tablebody").on("click",".btn_del",function(){
                    var rowid = parseInt($(this).parent().parent().children('.rowid').text());
                    notie.confirm('确定要删除?', 'Yes', 'No', function() {
                        $.post('./deleteContri', {rowid: rowid}, function(data, status) {
                            if(status!="success"){
                                alert("数据连接出错");
                            }else{
                                if(data==1){
                                    notie.alert(1, '删除成功，正在跳转...', 1);
                                    setTimeout(function(){
                                        window.location.reload();
                                    }, 2000);
                                }else{
