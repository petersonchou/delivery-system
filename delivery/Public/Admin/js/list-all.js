...
function list_dynamic(dynamic, pagenow, pagesize){
    ...
    for(var i=start_row-1; i<end_row; i++){
        if(dynamic[i]["statusid"]==1){
            var part = "<td style='color:#F68A1E'><b>待审核</b></td><td><button class='btn btn-default btn_look' title='查看'><i class='icon-camera'></i></button><button class='btn btn-default btn_edit btn_css' title='编辑'><i class='icon-edit'></i></button><button class='btn btn-default btn_del btn_css' title='删除'><i class='icon-trash'></i></button></td></tr>";
        }else if(dynamic[i]["statusid"]==2){
            var part = "<td style='color:red;font-size:14px'><b>未通过</b></td><td><button class='btn btn-default btn_look' title='查看'><i class='icon-camera'></i></button><button class='btn btn-default btn_css disabled'><i class='icon-lock'></i></button><button class='btn btn-default btn_del btn_css' title='删除'><i class='icon-trash'></i></button></td></tr>";
        }else if(dynamic[i]["statusid"]==3){
            var part = "<td style='color:#59B259'><b>已通过</b></td><td><button class='btn btn-default btn_look' title='查看'><i class='icon-camera'></i></button><button class='btn btn-default btn_css disabled'><i class='icon-lock'></i></button><button class='btn btn-default btn_css disabled'><i class='icon-lock'></i></button></td></tr>";
        }
        var title = dynamic[i]["title"];
        if(dynamic[i]["title"].length>10){
            title = title.substring(0, 10) + "...";
        }
        var row = $("<tr><td class='rowid' style='display:none'>"+dynamic[i]["id"]+"</td><td>"+(i+1)+"</td><td>"+title+"</td><td>"+"单位"+"</td><td>"+dynamic[i]["author"]+"</td><td>"+dynamic[i]["postdate"]+"</td>"+part);
...
function select_dynamic(dynamic, key){
    var new_arr = new Array();
    var index = 0;
    for(var i=0; i<dynamic.length; i++){
        if(dynamic[i]["statusid"]==key){
...
function search_dynamic(dynamic, keyword){
    ...
    var index = 0;
    for(var i=0; i<dynamic.length; i++){
        if(dynamic[i]["id"].match(keyword)!=null || dynamic[i]["title"].match(keyword)!=null || dynamic[i]["author"].match(keyword)!=null || dynamic[i]["postdate"].match(keyword)!=null ){
            new_arr[index] = dynamic[i];
            index = index + 1;
        }
    }
