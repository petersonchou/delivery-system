...
function list_dynamic(dynamic, pagenow, pagesize){
    ...
    if(dynamic.length==0){
        ...
        for(var i=start_row-1; i<end_row; i++){
            var part = "<td style='color:#59B259'><b>已通过</b></td><td><button class='btn btn-default btn_look' title='查看'><i class='icon-camera'></i></button>&nbsp;&nbsp;<button class='btn btn-default btn_recover' title='重审'><i class='icon-hand-up'></i></button>&nbsp;&nbsp;<button class='btn btn-default btn_refuse' title='拒绝'><i class='icon-trash'></i></button></td></tr>";
...
function checkStrLen_add(){
    ...
    tex.focus(function(){
        if($(this).val()==""){
            $("#modal_add .tip").html("您总共可以输入700个字");
        }
    });
    if(ie){
        tex[0].oninput = changeNum;
