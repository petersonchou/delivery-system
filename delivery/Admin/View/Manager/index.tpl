...
<html class="no-js">
    <head>
        ...
        <link href="{$Think.const.ELSE_URL}bootstrap-switch.min.css" rel="stylesheet">
        <style>
            thead{
    ...
    <body>
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <div style="text-align:center">
                                ...
                                状态：<span class="modal-title-status" style="color:#F68A1E;font-weight:bold;"></span>
                            </div>
                        </div>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" style="float:left" class="btn btn-primary btn_checked">审核</button>
                            <button type="button" style="float:left" class="btn btn-default btn_busy" data-dismiss="modal">再看看</button>
        ...
        <script src="{$Think.const.ADMIN_JS_URL}list-wait.js"></script>
        <script src="{$Think.const.ELSE_URL}notie/notie.min.js"></script>
        <script src="{$Think.const.ELSE_URL}bootstrap-switch.min.js"></script>
        <script>
            $(function(){
            	//页面初始加载
                $(".sort").val("default");
                ...
                $(".tablebody").on("click",".btn_look",function(){
                    ...
                    $.fn.bootstrapSwitch.defaults.offText = '拒绝';
                ...
                //当模态框关闭时将开关还原
                $("#modal_look").on("hidden.bs.modal",function(){
                    $('input[name="myswitch"]').bootstrapSwitch('state', true, true);
                });

                //点击批阅按钮
                $("#modal_look .btn_checked").click(function(e){
                    layer.msg('确定要审核？', {
                      time: 0 //不自动关闭
                      ,btn: ['是', '否']
                      ,yes: function(index){
                        layer.close(index);
                        var rowid = parseInt($(".check_rowid").text());
                        if(checkstatus){
                            var status = 3;
                        }else{
                            var status = 2;
                        ...
                        $.post('./updStatus', {id: rowid, status: status}, function(data, mystatus){
                            if(mystatus!="success"){
                                ...
                                if(data==1){
                                    $("#modal_look").modal("hide");
                                    notie.alert(1, '审核成功，正在跳转...', 1);
                                    setTimeout(function(){
